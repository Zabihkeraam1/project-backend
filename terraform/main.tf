    ...
    code_repository {
      ...
      code_configuration {
        ...
        #   build_command = "npm --prefix ./backend install --production"
          build_command = "python3.11 -m pip install --upgrade pip && python3.11 -m pip install --system uvicorn fastapi && [ -f ./backend/requirements.txt ] && python3.11 -m pip install --system -r ./backend/requirements.txt"
        #   start_command = "node ./backend/server.js" 
          start_command = "python3.11 uvicorn backend.main:app --host 0.0.0.0 --port 8080" 
          port           = 8080
